...
public interface LoanRepository extends JpaRepository<Loan, Long> {
    ...
    @Query("select new com.ssafy.teentech.loan.dto.response.LoanSummaryResponseDto(l.loanId, l.title, l.amount, l.balance, l.approvalDate) from Loan l where l.user = :user and l.balance > 0 and l.state = :state")
    List<LoanSummaryResponseDto> findAllByUserAndBalanceIsGreaterThanZeroAndState(@Param("user") User user, @Param("state")
        State state);
...
//    List<LoanHistoryResponseDto> findAllByUserAndBalanceIsEqualToZero(@Param("user") User user);
    ...
    List<LoanHistoryResponseDto> findAllByUserAndRejectOrCompleteOrFail(@Param("user") User user, @Param("reject") State reject, @Param("complete") State complete, @Param("fail") State fail);
